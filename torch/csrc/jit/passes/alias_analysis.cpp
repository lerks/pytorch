...
std::unordered_set<const Value*> AliasDb::getReads(Node* n) const {
...
class AliasDb::WorkingSet {
 public:
  ...
  void add(Node* n) {
    ...
    for (const auto& write : aliasDb_.getWrites(n)) {
      writes_.insert(write);
    }
    for (const auto& read : aliasDb_.getReads(n)) {
      reads_.insert(read);
    }
  ...
  void eraseMover() {
    ...
    for (const auto& write : aliasDb_.getWrites(mover)) {
      const auto it = writes_.find(write);
      if (it != writes_.end()) {
        writes_.erase(it);
      }
    }
    for (const auto& read : aliasDb_.getReads(mover)) {
